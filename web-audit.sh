...
do
     capture-website $(echo $i) --scale-factor=1 --overwrite --output=$(echo $GUGULETHU_SCREENSHOT)$(basename $i)\ $(date +%Y%m%d).jpg --no-default-background --type=jpeg --timeout=100000 --delay=20
     capture-website $(echo $i) --scale-factor=1 --overwrite --output=$(echo $GUGULETHU_SCREENSHOT)$(basename $i)\[m\]\ $(date +%Y%m%d).jpg --emulate-device="iPhone X" --no-default-background --type=jpeg --timeout=100000 --delay=20
done

# Run screenshots on all pages on sizakelegumede.co.za
capture-website https://sizakelegumede.co.za --scale-factor=1 --overwrite --output=$(echo $SIZAKELE_SCREENSHOT)$home\ $(date +%Y%m%d).jpg --no-default-background --type=jpeg --timeout=100000 --delay=20
capture-website https://sizakelegumede.co.za --scale-factor=1 --overwrite --output=$(echo $SIZAKELE_SCREENSHOT)$home\[m\]\ $(date +%Y%m%d).jpg --emulate-device="iPhone X" --no-default-background --type=jpeg --timeout=100000 --delay=20
